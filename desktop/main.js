...
function startPythonAgent() {
  ...
  console.log('[Agent] Starting Python agent from', agentPath);
  ...
  pythonAgentProcess.stdout.on('data', (data) => {
    ...
    if (lines) console.log('[Agent]', lines);
  });
  pythonAgentProcess.stderr.on('data', (data) => {
    const lines = String(data).trim();
    if (lines) console.log('[Agent]', lines);
  });
  pythonAgentProcess.on('error', (err) => {
    console.warn('[Agent] Failed to start:', err.message);
    pythonAgentProcess = null;
  });
  pythonAgentProcess.on('exit', (code, signal) => {
...
async function createWindow() {
  ...
  // 注册 AI Agent 播放控制回调：通过 executeJavaScript 在渲染进程中执行切歌等操作
  if (typeof localServer.setPlaybackHandler === 'function') {
    localServer.setPlaybackHandler((jsExpr) => {
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.executeJavaScript(jsExpr).catch((err) => {
          console.warn('[AgentPlayback] executeJS error:', err.message);
        });
      }
    });
  }
...
if (!gotSingleInstanceLock) {
  ...
  app.on('before-quit', () => {
    ...
    if (pythonAgentProcess) {
      console.log('[Agent] Killing Python agent process');
