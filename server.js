...
const server = http.createServer(async (req, res) => {
  ...
  // ---------- 推荐歌曲 ----------
  if (pn === '/api/recommend/songs') {
    try {
      const info = await getLoginInfo();
      if (!info.loggedIn) { sendJSON(res, { loggedIn: false, recommend: [] }); return; }
      const r = await recommend_songs({ cookie: userCookie, timestamp: Date.now() });
      const body = r.body || r || {};
      const raw = body.recommend || (body.data && body.data.dailySongs) || (body.data && body.data.recommend) || [];
      const songs = raw.map(mapSongRecord).filter(s => s.id);
      sendJSON(res, { loggedIn: true, recommend: songs, body });
    } catch (err) {
      console.error('[RecommendSongs]', err);
      sendJSON(res, { error: err.message, recommend: [] }, 500);
    }
    return;
  }

  // ---------- 推荐资源（歌单） ----------
  if (pn === '/api/recommend/resource') {
    try {
      const info = await getLoginInfo();
      if (!info.loggedIn) { sendJSON(res, { loggedIn: false, recommend: [] }); return; }
      const r = await recommend_resource({ cookie: userCookie, timestamp: Date.now() });
      const body = r.body || r || {};
      const raw = body.recommend || (body.data && body.data.recommend) || [];
      const playlists = raw.map(pl => ({
        id: pl.id,
        name: pl.name,
        cover: pl.picUrl || pl.coverImgUrl || '',
        trackCount: pl.trackCount || 0,
        playCount: pl.playCount || 0,
        creator: (pl.creator && pl.creator.nickname) || '',
      })).filter(pl => pl.id);
      sendJSON(res, { loggedIn: true, recommend: playlists, body });
    } catch (err) {
      console.error('[RecommendResource]', err);
      sendJSON(res, { error: err.message, recommend: [] }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/song/detail') {
    try {
      const id = url.searchParams.get('id');
      if (!id) { sendJSON(res, { error: 'Missing song id' }, 400); return; }
      const r = await song_detail({ id, cookie: userCookie, timestamp: Date.now() });
      const body = r.body || r || {};
      const songs = body.songs || (body.data && body.data.songs) || [];
      const song = songs[0] || body.song || body || {};
      sendJSON(res, {
        id: song.id || id,
        name: song.name || '',
        ...
        album: song.al ? {
          ...
          picUrl: song.album.picUrl || song.album.cover || '',
        } : null),
        duration: song.dt || song.duration || 0,
        fee: song.fee || 0,
        popularity: song.pop || song.popularity || 0,
        no: song.no || 0,
  ...
  // ---------- AI Agent: 聊天转发 (SSE 透传) ----------
  if (pn === '/api/agent/chat' && req.method === 'POST') {
    (async () => {
      try {
        const body = await readRequestBody(req);

        // 前端会自行在 context 中发送 currentSong/queue/playing 等播放状态
        // 服务器只附加 loginStatus
        body.context = {
          ...(body.context || {}),
          loginStatus: userCookie ? 'logged_in' : 'not_logged_in',
        ...
        const agentUrl = 'http://127.0.0.1:3001/chat';
        ...
        if (!agentResp.ok) {
          const errText = await agentResp.text().catch(() => 'Unknown error');
          sendJSON(res, {
            error: 'AI 服务暂时不可用',
            detail: errText,
  ...
  // ---------- AI Agent: 播放控制 (支持下一首/上一首/暂停播放) ----------
  if (pn === '/api/agent/playback/next' && req.method === 'POST') {
    handleAgentPlaybackJS('nextTrack()', res);
    return;
  }
  if (pn === '/api/agent/playback/prev' && req.method === 'POST') {
    handleAgentPlaybackJS('prevTrack()', res);
    return;
  }
  if (pn === '/api/agent/playback/toggle-play' && req.method === 'POST') {
    handleAgentPlaybackJS('togglePlay()', res);
    return;
  }
  if (pn === '/api/agent/playback/play' && req.method === 'POST') {
    readRequestBody(req).then(function(data) {
      // readRequestBody 已返回 JSON.parse 后的对象，勿重复 JSON.parse
      if (!data || !data.song || !data.song.name) {
        sendJSON(res, { ok: false, error: '缺少歌曲信息' }, 400);
        return;
      }
      var js = 'agentPlaySong(' + JSON.stringify(data.song) + ')';
      handleAgentPlaybackJS(js, res);
    }).catch(function(err) {
      sendJSON(res, { ok: false, error: err.message }, 500);
    });
    return;
  }
  if (pn === '/api/agent/playback/queue' && req.method === 'POST') {
    readRequestBody(req).then(function(data) {
      if (!data || !Array.isArray(data.songs) || !data.songs.length) {
        sendJSON(res, { ok: false, error: '缺少歌曲列表' }, 400);
        return;
      }
      var js = 'agentBatchPlaySongs(' + JSON.stringify(data.songs) + ')';
      handleAgentPlaybackJS(js, res);
    }).catch(function(err) {
      sendJSON(res, { ok: false, error: err.message }, 500);
    });
...
// ---------- AI Agent: 播放控制 (由 desktop/main.js 注入回调) ----------
let _agentPlaybackHandler = null;

/**
 * 注册播放控制回调，供 desktop/main.js 在加载 server.js 后调用。
 * @param {function(string): void} handler - 接收 JS 表达式字符串，在渲染进程中执行
 */
server.setPlaybackHandler = function (handler) {
  _agentPlaybackHandler = handler;
};

function handleAgentPlaybackJS(jsExpr, res) {
  try {
    if (typeof _agentPlaybackHandler === 'function') {
      _agentPlaybackHandler(jsExpr);
      sendJSON(res, { ok: true });
    } else {
      sendJSON(res, { ok: false, error: 'playback handler not registered' }, 503);
    }
  } catch (err) {
    console.error('[AgentPlayback]', err);
    sendJSON(res, { ok: false, error: err.message }, 500);
  }
}
